...
<head>
    <meta charset="utf-8"/>
    <title>quiz.io | Module Cybersécurité</title>
...
<?php
    ...
    $_SESSION["mod_id"] = 1; // IT sec module
?>
...
<?php
    ...
    if (isset($_POST['submitted']))
    {
        ...
        while ($row = $q->fetch())
        {
            ...
            if (isset($_POST["user_" . $q_id ]) && $good_ans != $user_ans) /* wrong answer from user */
            {
                $_SESSION["hl_red"] .= "user_" . $q_id . "-" . $user_ans . ",";
                $_SESSION["hl_green"] .= "user_" . $q_id . "-" . $good_ans . ",";
                $stats->add($q_id, ANS_INCORRECT);
            }
            else if (isset($_POST["user_" . $q_id])) /* good boy */
            {
                $_SESSION["hl_green"] .= "user_" . $q_id . "-" . $good_ans . ",";
                $nocorrect++;
                $stats->add($q_id, ANS_CORRECT);
            }
            else /* User didn't answer this question */
            {
